...
function ENT:PlayerUse(pl)
	if IGS.IsInventoryOverloaded(pl) then
		pl:Notify("У тебя слишком много предметов в инвентаре!")
	...
	if self.Busy or self.Removed then -- хз нужно ли именно здесь, но я добавил
		...
		if CurTime() - self.Busy > 5 then
			pl:Notify("Предмет в процессе перемещения в инвентарь")
			pl:Notify("Если процесс бесконечный, то поскорее сделайте скриншот или видеозапись и сообщите администратору")
	...
	IGS.AddToInventory(pl, UID, function(invDbID)
		...
		pl:Notify("Предмет помещён в донат-инвентарь")
